...
export function ForumLayout({ children, isAuthenticated = false, user }) {
  ...
  const effectiveIsAuthenticated = isAuthenticated || !!auth?.isAuthed
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-1">
              ...
              <NavLink
                to={auth?.user?.userId != null ? `/user/${auth.user.userId}` : '/user/u1'}
                icon={<User size={18} />}
                label="My Profile"
                active={isActive(auth?.user?.userId != null ? `/user/${auth.user.userId}` : '/user/u1')}
              />
